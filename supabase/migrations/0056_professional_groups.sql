-- RS KICKBOXING v0.144 - professional group identity, moderation and presence
...
drop policy if exists "rs_group_artwork_public_read" on storage.objects;
create policy "rs_group_artwork_public_read"
...
drop policy if exists "rs_group_artwork_staff_update" on storage.objects;
create policy "rs_group_artwork_staff_update"
...
as $$
    select
        ...
        (
            ...
            left join public.rs_profiles p4 on p4.id=m4.student_id
            where m4.group_id=g.id
              and not coalesce(p4.active=true and p4.last_seen_at>now()-interval '2 minutes',false)
        ),
        g.thumbnail_path,
        g.students_can_post,
...
revoke all on function public.rs_group_catalog() from public,anon;
...
begin
    ...
    insert into public.rs_groups(
        name,description,thumbnail_path,students_can_post,students_can_media,open_join,
...
create or replace function public.rs_staff_update_group_v2(
    ...
    p_students_can_post boolean,
    p_students_can_media boolean,
    p_open_join boolean
)
returns void
language plpgsql
...
begin
    ...
    select
        ...
        coalesce(p.email,''),
        p.avatar_path,
        coalesce(p.active=true and p.last_seen_at>now()-interval '2 minutes',false),
        gm.joined_at
    from public.rs_group_memberships gm
    join public.rs_profiles p on p.id=gm.student_id
    where gm.group_id=p_group_id
    order by
        coalesce(p.active=true and p.last_seen_at>now()-interval '2 minutes',false) desc,
...
begin
    ...
    if p_join then
        if not exists(
            ...
            where g.id=p_group_id and g.active=true and (g.open_join=true or (select private.rs_is_staff()))
        ) then
            raise exception 'group joining is closed by trainer' using errcode='42501';
        end if;
        ...
        delete from public.rs_group_memberships
        where group_id=p_group_id and student_id=v_uid;
    end if;
end;
$$;
...
begin
    ...
    select g.students_can_post,g.students_can_media
    into v_can_post,v_can_media
    from public.rs_groups g
    where g.id=p_group_id and (g.active=true or v_staff);

    if not found then raise exception 'group unavailable' using errcode='P0002'; end if;

    if not v_staff and not coalesce(v_can_post,true) then
        raise exception 'trainer has disabled student posting in this group' using errcode='42501';
    end if;
    if not v_staff and p_media_path is not null and not coalesce(v_can_media,true) then
        raise exception 'trainer has disabled student media in this group' using errcode='42501';
    end if;
...
revoke all on function public.rs_set_my_group_membership(uuid,boolean) from public,anon;
revoke all on function public.rs_send_group_message(uuid,text,text,text,text) from public,anon;
