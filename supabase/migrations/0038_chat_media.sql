...
-- Migration 0038: private coach/group chat media attachments.
...
alter table public.rs_coach_messages
    ...
    check (
        char_length(body) between 0 and 1200
        and (
            char_length(trim(body)) > 0
            or media_path is not null
        )
        and (media_kind is null or media_kind in ('IMAGE','VIDEO'))
    );
...
drop policy if exists "rs_group_messages_member_select" on public.rs_group_messages;
create policy "rs_group_messages_member_select"
...
drop policy if exists "rs_group_messages_member_insert" on public.rs_group_messages;
create policy "rs_group_messages_member_insert"
on public.rs_group_messages
for insert
to authenticated
with check (
...
drop policy if exists "rs_chat_media_insert" on storage.objects;
create policy "rs_chat_media_insert"
on storage.objects
for insert
to authenticated
with check (
...
drop policy if exists "rs_chat_media_select" on storage.objects;
create policy "rs_chat_media_select"
on storage.objects
for select
to authenticated
using (
...
begin
    ...
    if trim(coalesce(p_body,''))='' and p_media_path is null then
    ...
    values(
        ...
        left(coalesce(p_body,''),1200),
        p_media_path,p_media_kind,p_media_name
    )
...
begin
    ...
    values(
        p_group_id,v_uid,left(coalesce(p_body,''),1200),p_media_path,p_media_kind,p_media_name
    )
    returning id into v_id;
